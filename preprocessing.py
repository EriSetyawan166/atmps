...
for x in myresult:
    ...
    query = "SELECT id FROM tweet2 WHERE text_bersih=%s"
    ...
    # mycursor.execute("INSERT INTO tweet_bersih (id, user_screen_name, text, sentiment) VALUES (%s ,%s, %s, %s)", (x[0], x[1], bersih, x[3]))
        mycursor.execute("UPDATE tweet2 set text_bersih=%s WHERE id = %s", (bersih, x[0]))
    # UPDATE tweet2 SET sentiment=$hasil WHERE id=$id
    
    else:
        id_s = s[0][0]
        if(id_s != x[0]):
            mycursor.execute("DELETE FROM tweet2 WHERE id=%s", (x[0],))
